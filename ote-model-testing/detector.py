...
class Detector:
    ...
    def create_bbox(self, image, conf_treshold=0.5, overlay_treshold=0.5):
        ...
        class_indexes = detections['detection_classes'][0].numpy().astype(
            np.int32)
        ...
        bbox_idx = tf.image.non_max_suppression(
            bboxs, class_scores, max_output_size=50, iou_threshold=overlay_treshold, score_threshold=conf_treshold)
        ...
        if len(bbox_idx) != 0:
            for i in bbox_idx:
                ...
                xmin, xmax, ymin, ymax = (
                    xmin*im_w, xmax*im_w, ymin*im_h, ymax*im_h)
                xmin, xmax, ymin, ymax = int(xmin), int(
                    xmax), int(ymin), int(ymax)

                cv2.rectangle(image, (xmin, ymin), (xmax, ymax),
                              color=class_color, thickness=2)
                cv2.putText(image, display_text, (xmin, ymin-10),
                            cv2.FONT_HERSHEY_COMPLEX, 1, class_color, thickness=2)
